...
public final class DashMediaSource extends BaseMediaSource {
    ...
    private DashMediaSource(MediaItem mediaItem, DashManifest dashManifest, DataSource.Factory factory, ParsingLoadable.Parser<? extends DashManifest> parser, DashChunkSource.Factory factory2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, CmcdConfiguration cmcdConfiguration, DrmSessionManager drmSessionManager, LoadErrorHandlingPolicy loadErrorHandlingPolicy, long j, long j2) {
        ...
        this.simulateManifestRefreshRunnable = new Runnable() { // from class: androidx.media3.exoplayer.dash.DashMediaSource$$ExternalSyntheticLambda1
            @Override // java.lang.Runnable
            public final void run() {
                DashMediaSource.this.m198lambda$new$0$androidxmedia3exoplayerdashDashMediaSource();
            }
        };
    }
    ...
    public /* synthetic */ void m198lambda$new$0$androidxmedia3exoplayerdashDashMediaSource() {
        ...
        synchronized (this.manifestUriLock) {
            if (parsingLoadable.dataSpec.uri.equals(this.manifestUri) || (this.cmcdConfiguration != null && CmcdData.removeFromUri(parsingLoadable.dataSpec.uri).equals(this.manifestUri))) {
                if (this.manifest.location != null) {
                    removeFromUri = this.manifest.location;
                } else {
                    removeFromUri = CmcdData.removeFromUri(parsingLoadable.getUri());
                }
                this.manifestUri = removeFromUri;
            }
        }
    ...
    private void processManifest(boolean z) {
        ...
        refreshSourceInfo(new DashTimeline(this.manifest.availabilityStartTimeMs, j2, this.elapsedRealtimeOffsetMs, this.firstPeriodId, availableStartTimeInManifestUs - Util.msToUs(period.startMs), j4, j3, this.manifest, getMediaItem(), this.manifest.dynamic ? this.liveConfiguration : null));
        if (this.sideloadedManifest) {
            return;
        }
        this.handler.removeCallbacks(this.simulateManifestRefreshRunnable);
    ...
    */
    ...
    private static long getIntervalUntilNextManifestRefreshMs(DashManifest dashManifest, long j) {
        ...
        if (msToUs4 == C.TIME_UNSET || msToUs4 >= 5000000) {
            msToUs4 = 5000000;
        }
        for (int i = 0; i < period.adaptationSets.size(); i++) {
            ...
            if (!list.isEmpty() && (index = list.get(0).getIndex()) != null) {
                ...
                if (nextSegmentAvailableTimeUs > 0 && (nextSegmentAvailableTimeUs < msToUs4 - SilenceSkippingAudioProcessor.DEFAULT_MINIMUM_SILENCE_DURATION_US || (nextSegmentAvailableTimeUs > msToUs4 && nextSegmentAvailableTimeUs < msToUs4 + SilenceSkippingAudioProcessor.DEFAULT_MINIMUM_SILENCE_DURATION_US))) {
