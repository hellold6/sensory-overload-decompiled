...
public class FragmentStateManager {
    ...
    void resume() {
        ...
        if (focusedView != null && isFragmentViewChild(focusedView)) {
            ...
            if (FragmentManager.isLoggingEnabled(2)) {
                Log.v("FragmentManager", "requestFocus: Restoring focused view " + focusedView + " " + (requestFocus ? NotificationsService.SUCCEEDED_KEY : "failed") + " on Fragment " + this.mFragment + " resulting in focused view " + this.mFragment.mView.findFocus());
            }
        }
    ...
    public Bundle saveState() {
        ...
        if (this.mFragment.mState > -1) {
            ...
            Bundle m92lambda$attachController$4$androidxfragmentappFragmentManager = this.mFragment.mChildFragmentManager.m92lambda$attachController$4$androidxfragmentappFragmentManager();
            if (!m92lambda$attachController$4$androidxfragmentappFragmentManager.isEmpty()) {
                bundle.putBundle(CHILD_FRAGMENT_MANAGER_KEY, m92lambda$attachController$4$androidxfragmentappFragmentManager);
            }
            if (this.mFragment.mView != null) {
                saveViewState();
            }
