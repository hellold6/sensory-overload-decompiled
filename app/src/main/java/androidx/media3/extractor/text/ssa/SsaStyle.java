...
public final class SsaStyle {
    ...
    public static SsaStyle fromStyleLine(String str, Format format) {
        ...
        try {
            ...
            int parseAlignment = format.alignmentIndex != -1 ? parseAlignment(split[format.alignmentIndex].trim()) : -1;
            Integer parseColor = format.primaryColorIndex != -1 ? parseColor(split[format.primaryColorIndex].trim()) : null;
            Integer parseColor2 = format.outlineColorIndex != -1 ? parseColor(split[format.outlineColorIndex].trim()) : null;
            float parseFontSize = format.fontSizeIndex != -1 ? parseFontSize(split[format.fontSizeIndex].trim()) : -3.4028235E38f;
            boolean z3 = true;
            boolean z4 = false;
            if (format.boldIndex == -1 || !parseBooleanValue(split[format.boldIndex].trim())) {
                z = true;
                z3 = false;
            } else {
                z = true;
            }
            if (format.italicIndex == -1 || !parseBooleanValue(split[format.italicIndex].trim())) {
                z2 = false;
            } else {
                z2 = false;
                z4 = z;
            }
            if (format.underlineIndex != -1 && parseBooleanValue(split[format.underlineIndex].trim())) {
                z2 = true;
            }
            return new SsaStyle(trim, parseAlignment, parseColor, parseColor2, parseFontSize, z3, z4, z2, format.strikeoutIndex != -1 && parseBooleanValue(split[format.strikeoutIndex].trim()), format.borderStyleIndex != -1 ? parseBorderStyle(split[format.borderStyleIndex].trim()) : -1);
